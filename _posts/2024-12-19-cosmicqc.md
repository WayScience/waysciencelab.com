...
This paper gives a clue that there are methods out there for outlier detection, but it doesn't seem like many researchers are using them for their image-based profiles.
Maybe this is due to not having a simple pipeline specifically for CellProfiler outputs.
Who knows, but let's get back to the first ever attempt at single cell quality control, shall we?
